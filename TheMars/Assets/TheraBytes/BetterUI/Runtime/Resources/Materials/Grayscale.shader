Shader "BetterUI/Grayscale"
{
	Properties
	{
		...
		_ColorMask("Color Mask", Float) = 15[Toggle(UNITY_UI_ALPHACLIP)]
	...
	#pragma multi_compile __ FORCE_CLIP
		...
		fixed4 frag(v2_f IN) : SV_Target
		{
			...
			half4 color = lerp(origColor, greyColor, IN.amount.x);
			color.a = origColor.a;
...
#else
			if (_UseClipRect)
				color *= UnityGet2DClipping(IN.worldPosition.xy, _ClipRect);		if (_UseAlphaClip)
				clip(color.a - 0.001);
