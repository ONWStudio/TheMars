...
internal class CheckBundleDupeDependenciesV2 : BundleRuleBase
{
    ...
    private void BuildImplicitDuplicatedAssetsSet(IEnumerable<DuplicateResult> dupeResults)
    {
        foreach (DuplicateResult dupeResult in dupeResults)
        {
            ...
            if (!groupData.TryGetValue(ExtractData.WriteData.FileToBundle[dupeResult.DuplicatedFile], out List<string> assets))
            {
...
[InitializeOnLoad]
internal sealed class RegisterCheckBundleDupeDependenciesV2
{
    static RegisterCheckBundleDupeDependenciesV2() => AnalyzeSystem.RegisterNewRule<CheckBundleDupeDependenciesV2>();
}
#endif