...
public class TMPIconCreator : EditorWindow
{
    [MenuItem("Onw Studio/TMP Icon Creator")]
    private static void Init()
    {
        GetWindow<TMPIconCreator>().Show();
    }
    ...
    private static Color[] resizeTexture(Color[] originalPixels, int originalWidth, int originalHeight, int targetWidth, int targetHeight)
    {
    ...
    public void OnGUI()
    {
        _size = EditorGUILayout.IntSlider(new GUIContent("Width"), _size, 128, 2048);
        _divideCount = Mathf.Clamp(EditorGUILayout.IntField(new GUIContent("Height"), _divideCount), 1, 32);
