...
namespace Onw.Manager
{
    public abstract class LocalizationSingleton<T> : LocalizationTable where T : LocalizationSingleton<T>
    {
        ...
        protected string GetLocalizationSettingsName()
        {
            return "TheMarsLocalization";
        }
        ...
        private static string GetFilePathWithExtention(bool fullPath)
        {
            ...
            var prop = t.GetField("FILE_PATH", BindingFlags.Static | BindingFlags.GetField | BindingFlags.NonPublic) ?? throw new Exception($"No static Property 'FILE_PATH' in {t}");
