...
namespace EditorTool
{
    public static class EditorTool
    {
        ...
        public static void DrawDropdown<T>(string label, T item, Func<T, string> getSelectedValue, Action<T, string> setSelectedValue, IEnumerable<string> options, Action<T> setDefault = null)
        {
            ActionEditorHorizontal(() =>
            {
                EditorGUILayout.LabelField(label, GUILayout.ExpandWidth(true));
                if (EditorGUILayout.DropdownButton(
                    new GUIContent(string.IsNullOrEmpty(getSelectedValue(item)) ? label : getSelectedValue(item)),
                    FocusType.Keyboard))
                {
        ...
        public static void DrawDropdownFromKVPValue<T, Key>(string label, T item, Func<T, string> getSelectedValue, Action<T, KeyValuePair<Key, string>> setSelectedValue, Dictionary<Key, string> options, Action<T> setDefault = null)
        {
            ActionEditorHorizontal(() =>
            {
                EditorGUILayout.LabelField(label, GUILayout.ExpandWidth(true));
                if (EditorGUILayout.DropdownButton(
                    new GUIContent(getSelectedValue(item)),
                    FocusType.Keyboard))
                {
                    GenericMenu menu = new();
                    ...
                    {
                        menu.AddItem(
                            new GUIContent(option.Value),
                            getSelectedValue(item) == option.Value,
                            () => setSelectedValue(item, option));
                    }
