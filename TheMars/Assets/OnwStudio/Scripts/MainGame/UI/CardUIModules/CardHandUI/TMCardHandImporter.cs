...
namespace TMCardUISystemModules
{
    ...
    public sealed class TMCardHandImporter : MonoBehaviour
    {
        public int Stack => _cardQueue.Count;

        [SerializeField] private Queue<TMCardUIController> _cardQueue = new(10);

        public void PushCards(IEnumerable<TMCardUIController> cards)
            => cards.ForEach(_cardQueue.Enqueue);

        public void PushCard(TMCardUIController card)
            => _cardQueue.Enqueue(card);

        public List<TMCardUIController> GetCards(int count)
        {
            List<TMCardUIController> cards = new(count);
